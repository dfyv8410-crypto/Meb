...
## 2.2.3 — Защита фото (watermark) + харденинг загрузки + responsive-уточнения
- **Watermark «ГОДНАЯ МЕБЕЛЬ»**: логотип заказчика (Google Drive `1Yvww6vI_m1gcAesQDVIH2tbSH_PSgtvt`,
  md5 `56296948…`) байт-в-байт совпал с уже присутствующим `uploads/branding/meb-logo.png` —
  никакой замены/подмены не потребовалось.
- Новый раздел в админке «Настройки → Защита фото (watermark)»: вкл/выкл, загрузка/замена логотипа,
  непрозрачность (0–100%, по умолч. 25), размер (4–45%, по умолч. 22), позиция (9 якорей, по умолч.
  нижний правый), режим «Одиночный»/«Повторяющийся», живой предпросмотр на реальном фото мебели
  (`/api/v1/watermark/preview`, admin-only, ничего не пишет).
- Конвейер: watermark наносится в `meb_var_generate()` на готовое полотно (после RESIZE/CROP/FIT,
  до кодирования WebP). Оригиналы в `uploads/` никогда не изменяются (подтверждено md5 в тесте).
- Инвалидация кэша автоматическая: сигнатура `/img/var` теперь включает отпечаток настроек watermark
  (включая mtime/размер файла логотипа). Любая смена настроек → новые URL → регенерация вариантов;
  при выключенном watermark URL бит-в-бит совпадают с довотермарковыми (кэш не пересоздаётся зря).
- Upload-хараденинг (`api/v1/media.php`): реальный тип через finfo + getimagesize, лимит
  12000×12000 / 40 МП (защита от pixel-bomb), проверка SVG на содержимое (скрипты/iframe/foreignObject
  отвергаются), плюс исправлен авто-прогрев производных (передаётся site-rooted URL вместо
  абсолютного пути — раньше производные после загрузки фактически не создавались) и расширены размеры
  (480/640/768/960/1280 4:3 + 1280×720 16:9).
- Responsive-уточнения шаблонов: базовые кандидаты srcset расширены (…, 768, 1440, 1920); для
  мобильной карточной сетки `sizes` заменено на `calc(100vw - 40px)` (карточка 335px на 375px вместо
  100vw → на Retina браузер выбирает 768w, а не 960w); hero-фото (photoBand, detail-hero) получили
  `loading="eager"` + `fetchpriority="high"`.
- Доказано E2E-тестом (23/23): watermark on/off меняет сигнатуру и URL, зона логотипа отличается
  (BR=61.3 vs контроль BL=0), оригинал не тронут, превью без auth блокируется, text-as-jpeg и
  scripted-SVG отвергаются, валидный PNG принимается с генерацией производных, 9 страниц отдают 200,
  мобильный «полосочный» фикс прежний.
...
- Сервисы запускаются через `setsid` (своя сессия) и живут после выхода из оболочки —
  проверено многократными командами: php-сервер и БД не падают между вызовами,
  `start-dev.sh` повторно сообщает «already running».
